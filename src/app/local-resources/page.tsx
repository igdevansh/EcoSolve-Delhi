    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="space-y-6">
        {mockLocalResources.map((resource) => {
          // Logic to handle different contact formats
          const isSimplePhone = resource.contact && /^[+\d\s()-]+$/.test(resource.contact);
          const isEmail = resource.contact && resource.contact.includes('@');
          const emailRegex = /([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9_-]+)/;
          const emailInDetailsMatch = resource.details.match(emailRegex);
          const emailInDetails = emailInDetailsMatch ? emailInDetailsMatch[0] : null;
            ...
            <Card key={resource.id} className="shadow-lg overflow-hidden">
              ...
              <CardContent className="pt-6 space-y-3">
                ...
                <div>
                  ...
                  {resource.contact && !isSimplePhone && !isEmail && (
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  ...
                  {isSimplePhone && resource.contact && (
                     <Button variant="outline" size="sm" asChild>
                      <Link href={`tel:${resource.contact.replace(/\s/g, "")}`}>
                        <Phone className="mr-2 h-4 w-4" /> Call
                      </Link>
                    </Button>
                  )}
                  {emailInDetails && !isEmail && (
                     <Button variant="outline" size="sm" asChild>
                      <Link href={`mailto:${emailInDetails}`}>
                        <Mail className="mr-2 h-4 w-4" /> Email
                      </Link>
                    </Button>
                  )}
